...
def generate(root=Path("results/q1_sensitivity"),figure_dir=Path("figures/q1_sensitivity"),report=Path("reports/Q1_SENSITIVITY_REPORT.md")):
    ...
    lines=["# 第一问参数灵敏度结果与评测整改","",
           ...
           f"本次计算调用记录的源码提交为 `{manifest['code_commit'] or 'null（无Git环境）'}`，13个计算依赖的统一换行摘要为 `{manifest['source_digest']}`。导出前已逐文件核对当前源码与各组run一致；打包结果的后续提交可以不同，不能据此跳过源码检查。原始字节哈希和CRLF/LF规则随manifest提供，审核整改说明见[PR #5回应](Q1_PR5_REVIEW_RESPONSE.md)。", "",
           "## 1. 固定1800 s响应","",
