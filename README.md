...
小组建模与论文工作入口。第一问的有效径向数值基准及灵敏度已完成并合入main；第二问的变物性热湿耦合模型、表3–4、72 h `result2.xlsx`、论文图和验证归档已在 `feat/q2-coupled-drying` 完成。第三、四问及整篇论文仍需按清单推进。
...
4. 问题一、二的“输入—求解—验证—导出”链路已建立；后续从第二问归档继续实现问题三事件和问题四收缩域。
