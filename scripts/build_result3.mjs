...
const previews=arg('--previews','.scratch/q3/workbook_previews');
...
const payload=JSON.parse(await fs.readFile(arg('--payload','.scratch/q3/workbook_payload.json'),'utf8'));
...
if(JSON.parse(await fs.readFile(payload.verification_file,'utf8')).passed!==true)
  throw new Error('Numerical verification failed');
...
const a1=sheet.getRange('A1').values[0][0];
const last=payload.rows.length+1;
sheet.getRange(`A1:V${last}`).clear({applyTo:'contents'});
sheet.getRange('A1:V1').values=[[a1,...Array.from({length:21},(_,i)=>i/10)]];
sheet.getRange(`A2:V${last}`).values=payload.rows;
sheet.getRange(`B2:V${last}`).format.numberFormat='0.0000';
sheet.getRange(`A2:A${last}`).format.numberFormat='0.0000';
sheet.getRange('B1:V1').format.numberFormat='0.0';
sheet.getRange(`A1:V${last}`).format.font={name:'Microsoft YaHei',size:10};
...
console.log(JSON.stringify({output:payload.output,rows:payload.rows.length,columns:22,sha256:await digest(payload.output)}));
